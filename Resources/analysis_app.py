...
def _parse_dt(val: Any) -> Optional[datetime]:
    if not val:
        return None
    try:
        # Handle Cloud Logging RFC3339 and CloudTrail ISO8601
...
def _extract_region_from_resource_name(rn: str) -> Optional[str]:
    if not rn:
        return None
    # Try common GCP patterns: .../locations/<loc>/..., .../zones/<zone>/..., .../regions/<region>/...
...
def _normalize_aws(event: Dict[str, Any], source_file: str) -> Dict[str, Any]:
    ...
    # CloudTrailEvent is a JSON string with additional details
    ...
    if not res_name:
        # Try to infer from requestParameters if present
...
def _load_run_dir(run_dir: Path) -> Tuple[List[Dict[str, Any]], Dict[str, List[Dict[str, Any]]]]:
    """Load all JSON files from a run directory and normalize records.

    Returns a list of normalized records and a mapping of filename -> raw list for details.
    """
    ...
    # Drop records without action or time entirely if everything is None
...
selected_path = None
# Prefer run_dir passed via session_state from ingestion app
...
# Display a simple chronological list and a density chart
st.write("Chronological list of events:")
st.dataframe(df_sorted[cols], use_container_width=True, height=300)

try:
    import altair as alt

    # Resample counts by 15 minutes for visualization
    tmp = df_sorted.copy()
...
if sel_file:
    ...
    # Option to view a specific record
